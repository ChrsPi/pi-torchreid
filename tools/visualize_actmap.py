        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            if outputs.dim() != 4:
                raise ValueError(
                    ...
                    f"shape of (b, c, h, w), i.e. 4 dimensions, but got {outputs.dim()} dimensions. "
                    "Please make sure you set the model output at eval mode "
                    "to be the last convolutional feature maps"
                )
            ...
            for j in range(outputs.size(0)):
                ...
                # RGB image
                img = imgs[j, ...]
                for t, m, s in zip(img, img_mean, img_std, strict=False):
