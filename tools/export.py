        ...
        if simplify:
            try:
                ...
                model_onnx, check = onnxsim.simplify(
                    model_onnx, dynamic_input_shape=dynamic, input_shapes={"t0": list(im.shape)} if dynamic else None
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--include", nargs="+", default=["onnx", "openvino", "tflite"], help="onnx, openvino, tflite")
    ...
    if onnx:
        f = export_onnx(
            extractor.model.eval(), im, args.weights, 12, train=False, dynamic=args.dynamic, simplify=True
        )  # opset 12
    if openvino:
