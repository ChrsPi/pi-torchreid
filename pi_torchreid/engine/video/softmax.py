...
from pi_torchreid.engine.image import ImageSoftmaxEngine
...
class VideoSoftmaxEngine(ImageSoftmaxEngine):
    ...
    Examples::
        ...
        datamanager = pi_torchreid.data.VideoDataManager(
    ...
    def __init__(
        self, datamanager, model, optimizer, scheduler=None, use_gpu=True, label_smooth=True, pooling_method="avg"
    ):
        super().__init__(datamanager, model, optimizer, scheduler=scheduler, use_gpu=use_gpu, label_smooth=label_smooth)
    ...
    def extract_features(self, input):
        ...
        features = torch.mean(features, 1) if self.pooling_method == "avg" else torch.max(features, 1)[0]
        return features
