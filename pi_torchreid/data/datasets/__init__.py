...
def init_image_dataset(name, **kwargs):
    ...
    if name not in avai_datasets:
        raise ValueError(f'Invalid dataset name. Received "{name}", but expected to be one of {avai_datasets}')
...
def register_image_dataset(name, dataset):
    ...
    if name in curr_datasets:
        raise ValueError(f"The given name already exists, please choose another name excluding {curr_datasets}")
...
def register_video_dataset(name, dataset):
    ...
    Examples::
        ...
        import pi_torchreid
        import NewDataset
        pi_torchreid.data.register_video_dataset('new_dataset', NewDataset)
        # single dataset case
