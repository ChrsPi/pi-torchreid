...
from pi_torchreid.utils.logging_config import logger

try:
    from pi_torchreid.metrics.rank_cylib.rank_cy import evaluate_cy

    IS_CYTHON_AVAI = True
except ImportError:
    IS_CYTHON_AVAI = False
    warnings.warn(
        "Cython evaluation (very fast so highly recommended) is unavailable, now use python evaluation.", stacklevel=2
    )
