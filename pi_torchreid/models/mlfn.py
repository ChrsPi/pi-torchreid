...
class MLFNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, fsm_channels, groups=32):
        ...
        self.fm_conv2 = nn.Conv2d(
            mid_channels, mid_channels, 3, stride=stride, padding=1, bias=False, groups=self.groups
        )
...
class MLFN(nn.Module):
    ...
    def __init__(self, num_classes, loss="softmax", groups=32, channels=None, embed_dim=1024, **kwargs):
        if channels is None:
